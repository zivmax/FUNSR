...
class Runner:
    def __init__(self, args, conf_path, mode="train"):
        ...
        self.discriminator = Discriminator(**self.conf["model.discriminator"]).to(
            self.device
        )
    ...
    def train(self):
        ...
        for iter_i in tqdm(range(res_step)):
            ...
            if self.iter_step % self.report_freq == 0:
                print_log(
                    "iter: {:8>d} total_loss = {} lr = {}".format(
                        self.iter_step,
                        total_loss,
                        self.sdf_optimizer.param_groups[0]["lr"],
                    ),
                    logger=logger,
                )
